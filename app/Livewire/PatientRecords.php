...
class PatientRecords extends Component
{
    ...
    {
        // Cache key for this patient (5-minute cache)
        $cacheKey = "patient_{$patientId}_details";
        
        // Try to get from cache first
        $patient = \Illuminate\Support\Facades\Cache::remember($cacheKey, 300, function() use ($patientId) {
        ...
        if ($patient) {
            // Map email_address to email for template compatibility
            $patient->email = $patient->email_address ?? null;
            $this->selectedPatient = (object) $patient;
            
            // Last visit query (not cached, gets fresh data)
            $this->lastVisit = DB::table('appointments')
                                ->where('patient_id', $patientId)
                                ->where('status', 'Completed')
                                ->orderBy('appointment_date', 'desc')
                                ->first(['id', 'appointment_date', 'service_id']);
        }
    }
    ...
    public function deletePatient($patientId)
    {
        try {
            ...
            \Illuminate\Support\Facades\Cache::forget("patient_{$patientId}_details");
    ...
    /**
     * Clear patient cache when updating
     */
    public function clearPatientCache($patientId)
    {
        \Illuminate\Support\Facades\Cache::forget("patient_{$patientId}_details");
    }
}